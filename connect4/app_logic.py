...
def create_game(json: dict) -> Tuple[dict, int]:
    _req = models.CreateGameRequest(json)
...
def join_game(json: dict) -> Tuple[dict, int]:
...
def get_game(gameId: str, playerId: str) -> Tuple[dict, int]:
    ...
    player = database.get_player(gameId, playerId)
    if player is None:
...
def make_move(json: dict) -> Tuple[dict, int]:
    ...
    player = database.get_player(req.gameId, req.playerId)
    if player is None:
        raise exceptions.ApiException("Player not found", 404)
